...
from scripts.proto.autoencoder_helpers import makedirs, list_of_distances, print_and_write, list_of_norms
...
model_folder = os.path.join(os.getcwd(), "../../../saved_data/saved_model", "mnist_model", "mnist_cae_1")
...
# Network Parameters
n_prototypes = 10  # the number of prototypes
n_layers = 4
...
hidden_dim = 512
weights = {
    'enc_f1': tf.Variable(tf.random_normal([input_size, hidden_dim],
                                           stddev=0.01,
                                           dtype=tf.float32),
                          name='encoder_f1'),
    'enc_f2': tf.Variable(tf.random_normal([hidden_dim, hidden_dim],
                                           stddev=0.01,
                                           dtype=tf.float32),
                          name='encoder_f2'),
    'enc_f3': tf.Variable(tf.random_normal([hidden_dim, hidden_dim],
                                           stddev=0.01,
                                           dtype=tf.float32),
                          name='encoder_f3'),
    'enc_f4': tf.Variable(tf.random_normal([hidden_dim, n_map_4],
                                           stddev=0.01,
                                           dtype=tf.float32),
                          name='encoder_f4'),
    'dec_f4': tf.Variable(tf.random_normal([n_map_4, hidden_dim],
                                           stddev=0.01,
                                           dtype=tf.float32),
                          name='decoder_f4'),
    'dec_f3': tf.Variable(tf.random_normal([hidden_dim, hidden_dim],
                                           stddev=0.01,
                                           dtype=tf.float32),
                          name='decoder_f3'),
    'dec_f2': tf.Variable(tf.random_normal([hidden_dim, hidden_dim],
                                           stddev=0.01,
                                           dtype=tf.float32),
                          name='decoder_f2'),
    'dec_f1': tf.Variable(tf.random_normal([hidden_dim, input_size],
                                           stddev=0.01,
                                           dtype=tf.float32),
                          name='decoder_f1')
}
...
biases = {
    'enc_b1': tf.Variable(tf.zeros([hidden_dim], dtype=tf.float32),
                          name='encoder_b1'),
    'enc_b2': tf.Variable(tf.zeros([hidden_dim], dtype=tf.float32),
                          name='encoder_b2'),
    'enc_b3': tf.Variable(tf.zeros([hidden_dim], dtype=tf.float32),
                          name='encoder_b3'),
    'enc_b4': tf.Variable(tf.zeros([n_map_4], dtype=tf.float32),
                          name='encoder_b4'),
    'dec_b4': tf.Variable(tf.zeros([hidden_dim], dtype=tf.float32),
                          name='decoder_b4'),
    'dec_b3': tf.Variable(tf.zeros([hidden_dim], dtype=tf.float32),
                          name='decoder_b3'),
    'dec_b2': tf.Variable(tf.zeros([hidden_dim], dtype=tf.float32),
                          name='decoder_b2'),
    'dec_b1': tf.Variable(tf.zeros([input_size], dtype=tf.float32),
                          name='decoder_b1')
}
...
def fc_layer(input, weight, bias, nonlinearity=tf.nn.relu):
    inside_term = tf.matmul(input, weight) + bias
    if nonlinearity is None:
        return inside_term
    return nonlinearity(inside_term)


# construct the model
# eln means the output of the nth layer of the encoder
el1 = fc_layer(X, weights['enc_f1'], biases['enc_b1'])
el2 = fc_layer(el1, weights['enc_f2'], biases['enc_b2'])
el3 = fc_layer(el2, weights['enc_f3'], biases['enc_b3'])
el4 = fc_layer(el3, weights['enc_f4'], biases['enc_b4'], nonlinearity=None)
...
feature_vectors = tf.identity(el4, name='feature_vectors')

# the list prototype feature vectors
prototype_feature_vectors = tf.Variable(tf.random_uniform(shape=[n_prototypes, n_map_4],
                                                          dtype=tf.float32),
...
dl4 = fc_layer(feature_vectors, weights['dec_f4'], biases['dec_b4'])
dl3 = fc_layer(dl4, weights['dec_f3'], biases['dec_b3'])
dl2 = fc_layer(dl3, weights['dec_f2'], biases['dec_b2'])
dl1 = fc_layer(dl2, weights['dec_f1'], biases['dec_b1'], nonlinearity=tf.nn.sigmoid)
