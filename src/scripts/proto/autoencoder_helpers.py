...
def create_classification_fn(prototypes, weight_matrix):
    reshaped_p = np.reshape(prototypes, (-1, 2))
    num_prototypes = prototypes.shape[0]
    def classification_fn(enc):
        # Find distance to each of the prototypes.
        distances = np.zeros(num_prototypes)
        for i, prototype in enumerate(reshaped_p):
            dist = np.sum((enc - prototype) ** 2)
            distances[i] = dist
        weighted = np.matmul(distances, weight_matrix)
